...
function toggleLangMenu(event) {
  ...
  const langBtn = event.currentTarget;
...
function changeLanguage(lang) {
  ...
  // Close menu
  const langMenu = document.getElementById('langMenu');
  const langBtn = document.querySelector('.action-btn.active');
...
function getColorStyle(colorName) {
  ...
  // Specific Real-World Color Mappings
  const colorMap = {
    // Apple
    ...
    'arctic white': '#f0f8ff',
    'ultramarine': '#120a8f',
    'natural titanium': '#b3b0a7',
    'titanium black': '#4f4f4f',
    'titanium white': '#f2f2f2',
    'titanium blue': '#2f3b52',
    'desert titanium': '#cbbfa3',
    'rose titanium': '#b76e79',
    'mist blue': '#a0b6cd',
    'sage': '#778a75',
    'deep purple': '#4b0082',
    'space black': '#1d1d1f',
    ...
    'titanium violet': '#9932cc',
    'titanium yellow': '#ffd700',
    'onyx black': '#353839',
    'marble gray': '#b0c4de',
    'cobalt violet': '#8c52ff',
    'amber yellow': '#ffbf00',
    'phantom black': '#0e0e0e',
    'icy blue': '#b0e0e6',
    'graphite': '#4b4b4b',
    'mint': '#98ff98',
    'cream': '#fffdd0',
    ...
    'ripple blue': '#add8e6',
    'mighty black': '#000000',
    'submarine blue': '#000080',
    'navigator beige': '#f5f5dc',
    'explorer red': '#8b0000',
    'dreamy blue': '#b0e0e6',
    'aqua green': '#00ffff',
    'starry black': '#1a1a1a',
  ...
  // Partial matches for unknown specific colors
  if (lowerColor.includes('black') || lowerColor.includes('dark') || lowerColor.includes('midnight') || lowerColor.includes('obsidian') || lowerColor.includes('charcoal') || lowerColor.includes('graphite')) return '#000000';
  if (lowerColor.includes('white') || lowerColor.includes('light') || lowerColor.includes('starlight') || lowerColor.includes('snow') || lowerColor.includes('porcelain')) return '#ffffff';
  if (lowerColor.includes('blue') || lowerColor.includes('navy') || lowerColor.includes('azure') || lowerColor.includes('teal') || lowerColor.includes('sky') || lowerColor.includes('bay')) return '#3b82f6';
  if (lowerColor.includes('red') || lowerColor.includes('crimson') || lowerColor.includes('rose') || lowerColor.includes('burgundy')) return '#ef4444';
  if (lowerColor.includes('green') || lowerColor.includes('emerald') || lowerColor.includes('sage') || lowerColor.includes('lime') || lowerColor.includes('mint')) return '#10b981';
...
function getContrastColor(hexColor) {
  ...
  // Return black (text-gray-900) or white
...
function selectDetailColor(color, element) {
  ...
  // Revert all colors to neutral state
  ...
  if (element) {
    ...
    // Set background and calculating contrast text color
...
document.addEventListener('click', function (event) {
  ...
  const moreBtn = document.querySelector('.nav-more-btn');
  const langMenu = document.getElementById('langMenu');
  const langBtn = document.querySelector('[onclick="toggleLangMenu(event)"]');
  ...
  if (langMenu && langBtn && !event.target.closest('.nav-dropdown')) {
...
/**
 * Normalizes Arabic text to handle spelling variations (Alif, Ta Marbuta, etc.)
 */
function normalizeArabic(text) {
  ...
  return text.toString().toLowerCase().trim()
    ...
    .replace(/ة/g, 'ه')
    .replace(/ى/g, 'ي')
    .replace(/[^\u0621-\u064A\u0660-\u0669a-zA-Z0-0\s]/g, ''); // Remove diacritics and special chars
}
...
function searchProducts() {
  ...
  // If search is empty, go home or show all
  ...
  // Ensure products container is visible (especially if searching from home)
  toggleSections(false);

  // Search logic with normalization
  const filtered = products.filter(product => {
    const nameMatch = normalizeArabic(product.name).includes(normalizedSearchTerm);
    const nameArMatch = product.nameAr && normalizeArabic(product.nameAr).includes(normalizedSearchTerm);
    const brandMatch = normalizeArabic(product.brand).includes(normalizedSearchTerm);
    const specsMatch = product.specs.some(spec => normalizeArabic(spec).includes(normalizedSearchTerm));

    return nameMatch || nameArMatch || brandMatch || specsMatch;
  });
  ...
  updateSectionHeader(`${searchTitlePrefix} "${rawSearchTerm}"`, `${foundText} ${filtered.length} ${suffix}`);
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="no-results" style="grid-column: 1 / -1; text-align: center; padding: 40px; color: #6b7280;">
          <svg viewBox="0 0 24 24" style="width: 64px; height: 64px; margin-bottom: 20px; opacity: 0.5; stroke: currentColor; fill: none; stroke-width: 1.5;">
            <circle cx="11" cy="11" r="8"></circle>
            <path d="m21 21-4.35-4.35"></path>
          </svg>
          <h3 style="font-size: 20px; font-weight: 600; color: #1f2937; margin-bottom: 10px;">${translations[currentLang]['search-no-results'] || 'No products found'}</h3>
          <p>${translations[currentLang]['search-no-results-desc'] || 'Try different keywords or check your spelling.'}</p>
        </div>
      `;
    }
  } else {
